...
if __name__ == '__main__':
   ...
   from raisimGymTorch.env.bin import basketball_dribble
   ...
   env = VecEnv(basketball_dribble.RaisimGymEnv(home_path + "/rsc", dump(cfg['environment'], Dumper=RoundTripDumper)), cfg['environment'])
   print("env_created")
   env.setTask()
   ppo = RL(env, [128, 128])
   ...
   ppo.model_name = task_path + "/stats/hardfixedinit_ggtermination_distreward0.2_dribblereward0.8/"
   
   if not(os.path.isdir(ppo.model_name)):
      os.makedirs(ppo.model_name)
